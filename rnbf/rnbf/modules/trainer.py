...
from rnbf.datasets import (
    dataset, image_transforms, sdf_util, data_util
)
from rnbf.datasets.data_util import FrameData
from rnbf.modules import (
    fc_map, embedding, render, sample, loss
)
from rnbf import geometry, visualisation
from rnbf.eval import metrics, eval_pts
from rnbf.visualisation import draw, draw3D
from rnbf.eval.metrics import start_timing, end_timing
...
# from rnbf.modules.zoedepth.models.builder import build_model
# from rnbf.modules.zoedepth.utils.config import get_config
# from rnbf.modules.zoedepth.utils.misc import colorize
# import cv2
# # Global settings
# FINAL_HEIGHT = 256
# FINAL_WIDTH = 256
# OUTPUT_DIR = './output_test'  # Adjust output directory as needed
# DATASET = 'nyu'
...
class Trainer():
    def __init__(
        ...
        self.ros_subscriber = ros_subscriber  # Pass the ROSSubscriber instance
        self.device = device
        ...
        # self.model = AutoModelForDepthEstimation.from_pretrained("LiheYoung/depth-anything-base-hf").to(self.device)
    ...
    def get_data(self, idxs):
        ...
        for idx in idxs:
            sample = None  # Initialize sample as None
            # Keep looping until a valid sample is obtained
            while sample is None:
                sample = self.ros_subscriber.get_latest_data()
                if sample is None:
            ...
            im_np = sample["image"]
            image = im_np
            im_np_new_shape = ((0+1),) + im_np.shape
            im_np = im_np.reshape(im_np_new_shape)
            #print("--",im_np.shape)
            depth_np = sample["depth"]
            depth_np[np.isnan(depth_np)] = 0
            depth_or = depth_np
            # print("depth range", depth_np.shape, depth_np.max(), depth_np.min())

            ##--------------------
            depth_np_new_shape = ((0 + 1),) + depth_np.shape
            depth_np = depth_np.reshape(depth_np_new_shape)
            # print("depth range", depth_np.shape, depth_np.max(), depth_np.min())
            T_np = sample["T"]
            T_np_new_shape = ((0 + 1),) + T_np.shape
            T_np = T_np.reshape(T_np_new_shape)
            #print("im_np.shape",im_np.shape)
            Twc_o  = T_np.copy()
            Twc_o.flags.writeable = True  # Make sure the copy is writable

            # Convert the copy to a PyTorch tensor
            Twc_or = torch.from_numpy(Twc_o).float().to(self.device)

            ##*************************************************************  The main code dont touch for back up *******************************************************##
            ...
            # # flattened_pixels = im_np_copy.reshape(-1, 3)
            # #
            # # # Convert the pixel values to tuples so they can be used in a Counter
            # # pixel_tuples = [tuple(pixel) for pixel in flattened_pixels]
            # #
            ...
            # #
            # # # Print the most common color and its count
            # # print(f"The most regular color is {most_common_color[0]} with {most_common_color[1]} occurrences.")
            #
            # target_gray = np.array([155, 155, 155])
            # tolerance = 10
            #
            ...
            # B = im_np[0, :, :, 2]
            #
            # # Calculate the absolute differences from the target gray value
            # diff_r = np.abs(R - target_gray[0])
            # diff_g = np.abs(G - target_gray[1])
            # diff_b = np.abs(B - target_gray[2])
            #
            # # Find the pixels where the differences are within the tolerance
            # gray_mask = (diff_r <= tolerance) & (diff_g <= tolerance) & (diff_b <= tolerance)
            #
            # # Get the positions of the gray pixels
            # gray_positions = np.argwhere(gray_mask)
            #
            # # Create a depth image with shape (1, 720, 1280) and set all values to a default depth, e.g., 255
            # depth_image = depth_np.copy()
            # depth_or = depth_np.copy()
            #
            # # Set the depth values to 500 at the positions of the gray pixels
            # for pos in gray_positions:
            #     y, x = pos
            #     depth_image[0, y, x] = 0
            # import random
            #
            # # Check if all pixel values in the depth image are zero
            # # if np.all(depth_image == 0):
            # #     print("All depth values are zero. Setting a corner pixel to 1.")
            # #     for pos in gray_positions:
            # #         y, x = pos
            # #         depth_image[0, y, x] = 100
            #
            # # if np.all(depth_image == 0):
            # #     # print("All depth values are zero. ")
            # #     depth_image = self.depth_glob
            # #     T_np = self.T_np_glob
            # #     im_np = self.im_global
            # #
            # # else:
            # #     self.depth_glob = depth_image
            # #     self.T_np_glob = T_np
            # #     self.im_global = im_np
            #
            #     # Set a corner pixel to 1
            #     # depth_image[0, 500, 0] = 1000
            #
            # # Function to update and display the images
            # def update_display(original_depth, modified_depth, rgb_image, gray_mask):
            #     plt.clf()
            #
            #     plt.subplot(2, 2, 1)
            #     plt.title("Original Depth Image")
            #     im1 = plt.imshow(original_depth[0], cmap='viridis')
            #     plt.colorbar(im1, ax=plt.gca())
            #
            #     plt.subplot(2, 2, 2)
            #     plt.title("Modified Depth Image")
            #     im2 = plt.imshow(modified_depth[0], cmap='viridis')
            ...
            #
            #     # plt.subplot(2, 2, 4)
            #     # plt.title("Gray Pixels to be Removed")
            #     # gray_image = rgb_image[0].copy()
            #     # gray_image[gray_mask] = [255, 0, 0]  # Mark gray pixels with red color
            ...
            #
            # #
            # # depth_np_copy = depth_np.copy()  # Make a copy of the depth NumPy array
            # # depth_np_copy = depth_image.copy
            #
            # ###important now using modified depth rather than depth_np
            #
            # im_np_copy = im_np.copy()  # Make a copy of the NumPy array
            # im_np_copy.flags.writeable = True  # Make sure the copy is writable
            # # Convert the copy to a PyTorch tensor
            # im = torch.from_numpy(im_np_copy).float().to(self.device) / 255.
            #
            #
            #
            # depth_np_copy = depth_image.copy()
            # # update_display(im_np_copy, depth_np_copy, self.im_global, gray_mask)
            # # update_display( depth_np_copy, self.depth_glob, im_np_copy, self.im_global)
            # #
            # # plt.pause(5)
            # # print("hey")
            #
            #
            # depth_np_copy.flags.writeable = True  # Make sure the copy is writable
            # # Convert the copy to a PyTorch tensor
            # depth = torch.from_numpy(depth_np_copy).float().to(self.device)
            #
            # num_zero_depth_pixels = torch.sum(depth != 0).item()
            # # print(f"Number of zero depth pixels: {num_zero_depth_pixels}")
            #
            # # print("depth", depth.shape)
            # # if np.all(depth_np_copy == 0):
            # #     print("yes all depth zero")
            #
            # # if np.all(depth_np_copy == 0):
            # #     print("All depth pixels are zero.")
            # # else:
            # #     num_non_zero_depth_pixels = np.count_nonzero(depth_np_copy)
            # #     print(f"Number of non-zero depth pixels: {num_non_zero_depth_pixels}")
            #
            #
            # T_np_copy = T_np.copy()  # Make a copy of the transformation matrix NumPy array
            # T_np_copy.flags.writeable = True  # Make sure the copy is writable
            #
            # # Convert the copy to a PyTorch tensor
            # T = torch.from_numpy(T_np_copy).float().to(self.device)
            #
            # #print(im.shape,depth.shape,T.shape )
            # #
            # # data = FrameData(
            ...
            #
            # # target_gray = np.array([155, 155, 155])
            # target_gray = np.array([202, 204, 205])
            #
            # tolerance = 10
            #
            # # Extract the RGB channels
            ...
            # # # Find the pixels where the differences are within the tolerance
            # # gray_mask = (diff_r <= tolerance) & (diff_g <= tolerance) & (diff_b <= tolerance)
            # gray_mask = (R == target_gray[0]) & (G == target_gray[1]) & (B == target_gray[2])
            #
            # # Get the positions of the gray pixels
            # gray_positions = np.argwhere(gray_mask)
            #
            ...
            # #     depth_image[0, y, x] = 0
            #
            # depth_image[0, gray_positions[:, 0], gray_positions[:, 1]] = 0
            #
            #
            #
            #
            ...
            target_gray = torch.tensor([202, 204, 205], dtype=torch.float32, device=self.device) / 255.0
            tolerance = 10 / 255.0
            ...
            # Calculate the absolute differences from the target gray value
            diff_r = torch.abs(R - target_gray[0])
            diff_g = torch.abs(G - target_gray[1])
            diff_b = torch.abs(B - target_gray[2])

            # Find the pixels where the differences are within the tolerance
            gray_mask = (diff_r <= tolerance) & (diff_g <= tolerance) & (diff_b <= tolerance)

            # Update the depth values to 0 where gray pixels are detected
            depth[0, gray_mask] = 0
            depth_image = depth.cpu().numpy()

            # Count the number of zero depth pixels (for checking or further processing)
            # num_zero_depth_pixels = torch.sum(depth != 0).item()
            # print("num_zero_depth_pixels", num_zero_depth_pixels)
            ...
            # def display_depth_images(original, modified):
            #     # Convert tensors to numpy for plotting
            #     original_np = original.cpu().numpy().squeeze()  # Move to CPU, convert to numpy, and remove extra dimensions
            #     modified_np = modified.cpu().numpy().squeeze()
            #
            ...
            #
            #     # Display modified depth image
            #     axes[1].imshow(modified_np, cmap='gray')
            #     axes[1].set_title('Modified Depth Image')
            #     axes[1].axis('off')
            #     plt.savefig('/home/coslam/Rosbag_Record/depth_comparison.png')  # Save the figure to a file
            #     plt.close(fig)


            # Call the function with your tensors
            # display_depth_images(depth, depth_original)
    ...
    def is_keyframe(self, T_WC, depth_gt):
        ...
        num_zero_depth = torch.sum(depth_sample != 0).item()
        # print(f"Number of zero depth pixels**********************************: {num_zero_depth}")
    ...
    def select_keyframes(self):
        ...
        non_zero_indices = np.nonzero(loss_dist_np)[0]
        num_non_zero = len(non_zero_indices)
        # print("num_non_zero",num_non_zero, select_size)
        # it can handel but it not optimal may be assigning the last few frames or randomly beyween all key frames might be
        # also start turning earlier

        # this is causing some issue ------ change
        if select_size > num_non_zero:
            select_size = num_non_zero
        #
        ...
        last = n_frames - 1
        # print("last", last, "rand_ints", rand_ints)

        idxs = [*rand_ints, last - 1, last]
        # needed_indices = 5 - len(rand_ints)
        # additional_indices = [n_frames - 2 - i for i in range(needed_indices - 1, -1, -1)]
        #
        # idxs = list(rand_ints) + additional_indices
        # #print("idxs", idxs)
        return idxs
        #
        # return idxs
        # #if np.isnan(loss_dist_np):
        # if math.isnan(denom):
        #     last = n_frames - 1
        #     idxs = [last - 1, last]
        #
        ...
        #print("loss_dist_np------------------------",loss_dist_np)
        #
        ...
        #
        # # Replace NaN values with random values that sum to 1
        # def replace_nans(arr):
        #     nan_indices = np.isnan(arr)
        #     n_nans = np.sum(nan_indices)
        #
        #     if n_nans > 0:
        #         replacement_values = generate_random_values(n_nans)
        #         arr[nan_indices] = replacement_values
        #
        #     return arr
        #
        # # Example usage
        #
        # loss_dist_np = replace_nans(loss_dist_np)
        #
        # print("loss_dist_np------------------------", loss_dist_np)
    ...
    def sdf_eval_and_loss(
        ...
        # compute bounds
        def compute_hessian(inputs):
            hessian1 = vmap(vmap(hessian(self.sdf_map)))(inputs)
            return hessian1

        hessian_grad = compute_hessian(pc)
        import torch
        import torch.nn.functional as F

        def compute_mean_curvature(grad_F):
            """ Compute the mean curvature γ_mean. """
            norm_grad_F = torch.norm(grad_F, dim=-1, keepdim=True)
            normalized_grad_F = grad_F / norm_grad_F
            div_term = torch.autograd.grad(outputs=normalized_grad_F, inputs=grad_F,
                                           grad_outputs=torch.ones_like(normalized_grad_F),
                                           create_graph=True)[0]
            mean_curv = -torch.sum(div_term, dim=-1, keepdim=True) / norm_grad_F
            return mean_curv

        def curvature_regularization_loss(curv, k_curv):
            """ Compute the curvature regularization loss L_curv. """
            k_curv_tensor = torch.full_like(curv,
                                            k_curv)  # Create a tensor with the same shape as curv, filled with k_curv
            return torch.mean(torch.min(torch.abs(curv), k_curv_tensor))

        k_curv = 5
        gamma_mean = compute_mean_curvature(sdf_grad)
        curv_loss_mean = curvature_regularization_loss(gamma_mean, k_curv)

        # print("loss new curv_loss_mean", curv_loss_mean)

        def compute_curvatures(gradient, hessian, k_curv):
            # Compute the norm of the gradient
            grad_norm = torch.norm(gradient, dim=-1, keepdim=True)  # Shape: (m, n, 1)
            m, n, _ = gradient.shape
            # Compute the mean curvature gamma_mean
            div_term = torch.div(gradient, grad_norm)  # Shape: (m, n, 3)
            gamma_mean = -torch.div(torch.einsum('mnij,mnj->mni', hessian, div_term), grad_norm)  # Shape: (m, n, 1)

            # Compute the Gaussian curvature gamma_gauss
            H_adj = torch.linalg.pinv(hessian)  # Shape: (m, n, 3, 3)
            cross_term = torch.cross(gradient.unsqueeze(-1), H_adj, dim=-2)  # Shape: (m, n, 3, 3)
            cross_term = cross_term.view(m, n, 3, 3)  # Ensure the shape is (m, n, 3, 3)
            gamma_gauss = torch.div(
                torch.einsum('mnij,mnij->mn', cross_term, cross_term),
                grad_norm.squeeze(-1) ** 4
            )  # Shape: (m, n)

            return gamma_mean, gamma_gauss

        def compute_l_curvature(gradient, hessian, k_curv):
            gamma_mean, gamma_gauss = compute_curvatures(gradient, hessian, k_curv)
            # print("gamma_mean, gamma_gauss", gamma_mean.shape, gamma_gauss.shape)
            # Choose the desired curvature term (mean or gauss) based on application
            # gamma = gamma_mean.squeeze(-1)  # or gamma_gauss based on preference
            gamma = gamma_gauss
            # Convert k_curv to a tensor of the same shape as gamma
            k_curv_tensor = torch.full_like(gamma, k_curv)

            # Compute the regularization loss L_curv
            l_curv = torch.mean(torch.minimum(torch.abs(gamma), k_curv_tensor))  # Shape: ()

            return l_curv

        k_curv = 1  # Clipping value

        l_curv = compute_l_curvature(sdf_grad, hessian_grad, k_curv)
        # print(f"L_curv: {l_curv.item()}")
        ...
        total_loss, total_loss_mat, losses = loss.tot_loss(
            sdf_loss_mat, grad_loss_mat, eik_loss_mat,
            free_space_ixs, bounds, self.eik_apply_dist,
            self.trunc_weight, self.grad_weight, self.eik_weight, l_curv
        )
    ...
    def step(self):
        ...
        #print("self.frame",len(self.frames))
        ...
        T_WC_curr = self.frames.T_WC_curr
        #print("depth",depth_batch.shape)
        ...
        # n = T_WC_batch.shape[0] - 1  # You can change this to the desired first dimension
        n = T_WC_curr.shape[0] - 1  # You can change this to the desired first dimension

        # print("T", T_WC_batch)
        # Extract the elements (n, 4, 1), (n, 4, 2), and (n, 4, 3)
        # pts = T_WC_batch[n, 0:3, 3:]  # Indexing to get the desired elements
        pts = T_WC_curr[n, 0:3, 3:]  # Indexing to get the desired elements

        # Reshape the extracted elements into a (1, 12) tensor
        pts = pts.reshape(3)
        # print("pts---",pts)
        #print("sample_points",sample_pts)
        total_loss, losses, active_loss_approx, frame_avg_loss = \
            self.sdf_eval_and_loss(sample_pts, do_avg_loss=True)
        #change it to i
        frame_avg_loss = frame_avg_loss[:len(idxs)]
        # Shuffle frame_avg_loss to match the order of idxs
        frame_avg_loss = frame_avg_loss[np.argsort(idxs)]
        ...
        return losses, step_time, pts, T_WC_batch
    ...
    def eval_traj_cost(self, t_ahead=5.):
        ...
        if self.traj_file:
            ...
            with torch.set_grad_enabled(False):
                ...
                sdf = self.sdf_map(eval_pts)
                #print("eval point", eval_pts.shape)
                sdf = sdf.squeeze()
    ...
    def hessian_fn(self, pts):
        #grad = self.grad_fn(pts)
        pts = torch.FloatTensor(pts).to(self.device)
        #pts.requires_grad_()
        hessian = torch.autograd.functional.hessian(self.sdf_map, pts)
    ...
    def hessian_wthgrad(self, pts):
        ...
        hessian = torch.zeros((pts.shape[0], pts.shape[1], pts.shape[1])).to(self.device)

        for i in range(gradients.shape[1]):  # Loop over the gradient components (x, y, z)
    ...
    def hessian_direct(self, pts):
        pts = torch.FloatTensor(pts).to(self.device)
        # pts.requires_grad_()
        # pts.requires_grad_()
        # sdf = self.sdf_map(pts)
        # hessian1 = torch.autograd.functional.hessian(sdf, pts)

        hessian1 = torch.autograd.functional.hessian(self.sdf_map, pts)
        return hessian1.detach().cpu().numpy()

    def compute_hessian_f(self, pts):
        pts = torch.FloatTensor(pts).to(self.device)
        hessian1 = hessian(self.sdf_map)(pts)
        return hessian1.detach().cpu().numpy()