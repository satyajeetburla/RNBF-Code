...
from rnbf.geometry import transform
...
def huber_loss(input, delta=0.1):
    """
    Calculate the Huber Loss.
    """
    abs_input = torch.abs(input)
    quadratic_term = torch.min(abs_input, torch.tensor(delta))
    linear_term = abs_input - quadratic_term
...
def sdf_loss(sdf, bounds, t, loss_type="L1"):
    ...
    if loss_type == "L1":
        ...
        sdf_loss_mat = huber_loss(sdf_loss_mat)
    else:
...
def tot_loss(
    ...
    trunc_weight, grad_weight, eik_weight, l_curv
):
    ...
    # tot_loss = tot_loss + (0.0001 * l_curv.item())
