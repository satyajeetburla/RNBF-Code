...
def train(
    ...
    if rnbf_trainer.do_eval:
        ...
        if rnbf_trainer.mesh_eval:
            res['mesh_eval'] = {}
    if rnbf_trainer.do_vox_comparison:
        vox_res = {}

    last_eval = 0

    # live vis init--------------------------------------------------------------
    # if rnbf_trainer.live:
    kf_vis = None
    cv2.namedWindow('RNBF keyframes', cv2.WINDOW_AUTOSIZE)
    cv2.moveWindow("RNBF keyframes", 100, 700)

    # main  loop---------------------------------------------------------------
    print("Starting training for max", rnbf_trainer.n_steps, "steps...")
    size_dataset = len(rnbf_trainer.scene_dataset)

    break_at = -1

    t = 0
    for _ in range(rnbf_trainer.n_steps):
        # break at end -------------------------------------------------------
        if t == break_at and len(rnbf_trainer.eval_times) == 0:
            if save:
                if rnbf_trainer.save_slices:
        ...
        # get/add data---------------------------------------------------------
        finish_optim = \
            rnbf_trainer.steps_since_frame == rnbf_trainer.optim_frames
        if incremental and (finish_optim or t == 0):
            # After n steps with new frame, check whether to add it to kf set.
            if t == 0:
                add_new_frame = True
            else:
                add_new_frame = rnbf_trainer.check_keyframe_latest()
            ...
            if add_new_frame:
                ...
                if new_frame_id >= size_dataset:
                    ...
                    print("Total step time", rnbf_trainer.tot_step_time)
            ...
            if t == 0 or (rnbf_trainer.last_is_keyframe and not add_new_frame):
                kf_vis = visualisation.draw.add_im_to_vis(
                    kf_vis, rnbf_trainer.frames.im_batch_np[-1], reduce_factor=6)
                cv2.imshow('RNBF keyframes', kf_vis)
                cv2.waitKey(1)

        # optimisation step---------------------------------------------
        losses, step_time = rnbf_trainer.step()
        ...
        # visualisation----------------------------------------------------------
        if (
            not rnbf_trainer.live and update_im_freq is not None and
            (t % update_im_freq == 0)
        ):
            display = {}
            rnbf_trainer.update_vis_vars()
            display["keyframes"] = rnbf_trainer.frames_vis()
            # display["slices"] = rnbf_trainer.slices_vis()
            if show_obj:
        ...
        #view_freq = 10
        view_freq = 10
        if t % view_freq == 0 and rnbf_trainer.live:
            latest_vis = rnbf_trainer.latest_frame_vis()
            print(latest_vis[0].shape, latest_vis[1].shape,latest_vis[2].shape)
            # #cv2.imshow('RNBF (frame rgb, depth), (rendered normals, depth)', latest_vis[0])
            # cv2.imshow('RNBF (frame rgb, depth), (rendered normals, depth)', latest_vis[1])
            # #cv2.imshow('RNBF (frame rgb, depth), (rendered normals, depth)', latest_vis[2])
            ...
            cv2.imshow('RNBF (frame rgb, depth), (rendered normals, depth)', horizontal_concatenation)

            key = cv2.waitKey(5)

            # active keyframes vis
            # kf_active_vis = rnbf_trainer.keyframe_vis(reduce_factor=4)
            # cv2.imshow('RNBF keyframes v2', kf_active_vis)
            # cv2.waitKey(1)
        ...
        # save ----------------------------------------------------------------
        if save and len(rnbf_trainer.save_times) > 0:
            if rnbf_trainer.tot_step_time > rnbf_trainer.save_times[0]:
                save_t = f"{rnbf_trainer.save_times.pop(0):.3f}"
                print(
                    f"Saving at {save_t}s",
                    f" --  model {rnbf_trainer.save_checkpoints} ",
                    f"slices {rnbf_trainer.save_slices} ",
                    f"mesh {rnbf_trainer.save_meshes} "
                )
                    ...
                    torch.save(
                        {
                            "step": t,
                            "model_state_dict":
                                rnbf_trainer.sdf_map.state_dict(),
                            "optimizer_state_dict":
                                rnbf_trainer.optimiser.state_dict(),
                ...
                if rnbf_trainer.save_meshes and rnbf_trainer.tot_step_time > 0.4:
                    rnbf_trainer.write_mesh(mesh_path + f"/{save_t}.ply")

        # evaluation -----------------------------------------------------

        if len(rnbf_trainer.eval_times) > 0:
            if rnbf_trainer.tot_step_time > rnbf_trainer.eval_times[0]:
        ...
        if rnbf_trainer.do_eval and elapsed_eval > rnbf_trainer.eval_freq_s:
            ...
            if rnbf_trainer.sdf_eval and rnbf_trainer.gt_sdf_file is not None:
                ...
                print("Time ---------->", rnbf_trainer.tot_step_time)
...
if __name__ == "__main__":
    ...
    if save:
        ...
        save_path = "../../results/RNBF/" + time_str
        os.mkdir(save_path)
    else:
        save_path = None
